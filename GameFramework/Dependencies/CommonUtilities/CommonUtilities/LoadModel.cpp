...
std::string CU::LoadModel::GetWord(const char* aStr, int& aStartIndex) // returns the first word in the string and increments the index to the start of the next word
{
...
void CU::LoadModel::LoadModel(const char* aFile, Model& aModel) // loads a .ply model blenders obj format
{
	std::fstream modelFile(aFile);
	if (modelFile.fail() == true)
	{
		assert("blä");
	}
	...
	for (int i = 0; i < nrOfVertexes; ++i)
	{
		...
		//reads every value in the order found in the header of the file
			...
			{ 
				floatProperties[properties[j].myName].push_back(static_cast<float>(atof(currentNumber.c_str())));
			}
	...
	for (int i = 0; i < nrOfVertexes; ++i)
	{
		Voxel newVoxel(Vector3<float>(floatProperties["x"][i], floatProperties["z"][i], floatProperties["y"][i])*0.5f
			, Vector3<float>(float(uCharProperties["red"][i])/255.0f, float(uCharProperties["green"][i])/255.0f, float(uCharProperties["blue"][i])/255.0f),
			Vector3<float>(floatProperties["nx"][i], -floatProperties["nz"][i], floatProperties["ny"][i]));

		aModel.AddPoint(newVoxel);
	}
	aModel.CreateCullingMesh();
}